...
### Added
- `AGENTS.md` at repo root — agent-facing runbook that requires `/feature`, `/bug`, and `/checkpoint` runs to detect Prisma schema and/or seed changes and apply them to the production Neon branch *before* `git push`, with a verify step that spot-checks a `__data.json` payload (not just an HTTP 200) so empty-table failure modes are caught.
- `CLAUDE.md` symlinked to `AGENTS.md` so Claude Code picks up the same instructions without content duplication.
