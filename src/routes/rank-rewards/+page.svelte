<script>
  ...
  $: filteredLeaderboard = (data.rank?.leaderboard || []).filter(e =>
    e.time_period === filterPeriod
  );

  $: passes = data.rewards?.passes || [];
  $: activePasses = passes.filter(p => p.status === 'active');
  $: upcomingPasses = passes.filter(p => p.status === 'starts_soon');

  $: challenges = data.rank?.challenges || [];
  $: completedChallenges = challenges.filter(c => c.progress_fraction >= 1.0);
  $: hasUnlockedRewards = completedChallenges.length > 0;

  $: friendActivity = data.rank?.friendActivity || [];
  $: tierPerks = data.rewards?.tierPerks || {};
      ...
      <div class="summary-grid">
        <div class="summary-item main-score">
          <div class="score-display">
            <span class="large-score">{fan.xp_total.toLocaleString()}</span>
        ...
        <div class="summary-item">
          ...
          <div class="rank-display">
            <span class="rank-value">#{fan.rank}</span>
        ...
        <div class="summary-item">
          ...
          <div class="percentile-display">
            <span class="percentile-value">Top {fan.percentile}%</span>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="card unlock-next-card">
        ...
        <div class="unlock-next-progress">
          <div class="unlock-next-bar">
            <div class="unlock-next-fill" style="width: {((fan.xp_total - 5000) / (10000 - 5000)) * 100}%"></div>
          </div>
          <span class="unlock-next-fraction">{fan.xp_total.toLocaleString()} / 10,000 XP</span>
        </div>
      </div>
    </section>
  {/if}
